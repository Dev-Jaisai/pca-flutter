...
import '../screens/installments/this_month_screen.dart'; // ✅ ADD THIS
import '../screens/installments/universal_list_screen.dart';
...
class DashboardStatsState extends State<DashboardStats> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _playerEventsSubscription = EventBus().stream.listen((event) {
      if (['added', 'deleted', 'updated', 'installment_created',
        'installment_deleted', 'payment_recorded',
        'installment_updated', 'overdue_paid'].contains(event.action)) {
        _loadStats();
      }
    });
  }
  ...
  static Map<String, dynamic> _calculateStats(List<PlayerInstallmentSummary> rows) {
    ...
    for (var r in rows) {
      ...
      if (r.dueDate!.year == now.year &&
          ...
          !isPaid) due.add(r.playerId!);
      if (r.dueDate!.year == nextMonth.year &&
          r.dueDate!.month == nextMonth.month &&
          !isPaid) upcoming.add(r.playerId!);
    }
    return {
      'due': due.length,
      'upcoming': upcoming.length,
      'overduePlayers': overdue.length,
  ...
  Widget build(BuildContext context) {
    ...
    final configs = [
      ...
      },
      {
        ...
        // ✅ CHANGE: Navigate to ThisMonthScreen instead of UniversalListScreen
        'onTap': () => Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => const ThisMonthScreen())
        ),
      },
      {
        'title': 'Upcoming',
        ...
        // Navigate to Universal List for next month
        'onTap': () {
      ...
      },
      {
        ...
        'onTap': _overduePlayers > 0
            ...
            context,
            MaterialPageRoute(
                builder: (_) => const UniversalListScreen(
                    title: "Overdue List",
                    filterType: "OVERDUE"
                )
            )
