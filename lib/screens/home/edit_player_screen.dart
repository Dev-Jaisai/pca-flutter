...
class _EditPlayerScreenState extends State<EditPlayerScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('✅ Player Activated & Bill Generated!'),
        ...
        EventBus()
            .fire(PlayerEvent('installment_created')); // Navin bill banla ahe
      }
    } catch (e) {
      if (mounted)
      ...
      final Map<String, dynamic> data = {
        ...
        if (_newBillingDate != null)
          "firstInstallmentDate":
              _newBillingDate?.toIso8601String().split('T')[0],
      };
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        ...
        builder: (dialogContext, setDialogState) => AlertDialog(
          ...
          title: const Text("🏖️ Mark on Holiday",
              ...
              onPressed: () async {
                ...
                try {
                  await ApiService.pausePlayer(
                      widget.player.id, selectedDate, noteCtl.text);
  ...
  void _showActivateDialog() {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        ...
        builder: (dialogContext, setDialogState) => AlertDialog(
          backgroundColor: const Color(0xFF203A43),
          title: const Text("▶️ Welcome Back!",
              style: TextStyle(color: Colors.white)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                setState(() => _loading = true);
                try {
                  await ApiService.activatePlayer(
                      widget.player.id, selectedDate);
  ...
  // --- 🔥 FEATURE 3: MARK LEFT DIALOG ---
  void _showMarkLeftDialog() {
    DateTime selectedDate = DateTime.now();
    String selectedOption = 'COLLECT_FULL'; // Default matches Backend Enum
        ...
        builder: (dialogContext, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  "Future bills will be deleted. Select the date they left:",
                  style: TextStyle(color: Colors.white70, fontSize: 12),
                ),
                ...
                RadioListTile<String>(
                  title: const Text("Collect Full Fee",
                      style: TextStyle(color: Colors.white, fontSize: 14)),
                  subtitle: const Text("Keep original bill amount",
                ...
                // Option B: Partial Fee
                RadioListTile<String>(
                  title: const Text("Collect Partial Fee",
                      style: TextStyle(color: Colors.white, fontSize: 14)),
                  subtitle: const Text("Enter custom amount",
                      style: TextStyle(color: Colors.white54, fontSize: 10)),
                ...
                // If Partial selected, show text field
                if (showAmountField)
                  Padding(
                ...
                // Option C: Waive Off
                RadioListTile<String>(
                  title: const Text("Waive Off (Cancel Bill)",
                      style: TextStyle(color: Colors.white, fontSize: 14)),
                  subtitle: const Text("Set amount to 0",
                      style: TextStyle(color: Colors.white54, fontSize: 10)),
                  value: 'WAIVE_OFF',
                  groupValue: selectedOption,
      ...
      if (mounted) {
        ...
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text(errorMsg, style: const TextStyle(fontSize: 16)), // "Warning: Paid bills exist..."
            actions: [
                        ...
                        child: Container(
                          ...
                          child: Form(
                            ...
                            child: Column(
                              ...
                              children: [
                                _neonTextField(
                                    _nameCtl, 'Full Name', Icons.person),
                                const SizedBox(height: 16),
                                _neonTextField(_phoneCtl, 'Mobile', Icons.phone,
                                ...
                                const SizedBox(height: 16),
                                _neonTextField(_ageCtl, 'Age', Icons.cake,
                                    type: TextInputType.number),

                                // --- LIFECYCLE MANAGEMENT ---
                                const SizedBox(height: 30),
                                const Divider(color: Colors.white24),
                                const Text("Status & Lifecycle",
                                    style: TextStyle(
                                  ...
                                  SizedBox(
                                    ...
                                    child: OutlinedButton.icon(
                                      icon: const Icon(Icons.beach_access,
                                          size: 20),
                                      label:
                                          const Text("MARK ON HOLIDAY / PAUSE"),
                                      ...
                                      style: ElevatedButton.styleFrom(
                                          ...
                                          shape: RoundedRectangleBorder(
                                              borderRadius:
                                                  BorderRadius.circular(10))),
                                      onPressed:
                                          _showMarkLeftDialog, // 🔥 Opens Dialog
                                    ),
                                  ),
                                ] else
                                ...
                                const Divider(color: Colors.white24),
                                const SizedBox(height: 16),
                                _neonTextField(_notesCtl, 'Notes', Icons.note,
                                    maxLines: 2),
                                const SizedBox(height: 30),
  ...
  Widget _neonTextField(TextEditingController ctl, String label, IconData icon,
